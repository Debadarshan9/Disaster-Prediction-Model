...
def weekly_runoff_forecast(filename, wtd):
    ...
    scaled = scaler.fit_transform(daily[['weekly runoff']])
    scale = pd.DataFrame(scaled, columns=['weekly runoff'], index=daily.index)
    ...
    plt.title('Weekly Runoff Forecast')
...
# Example usage:
# weekly_runoff_forecast('your_filename', wtd=0)  # Set wtd to 0 for testing, 1 for future prediction
