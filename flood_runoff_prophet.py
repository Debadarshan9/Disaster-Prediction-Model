...
def flood_runoff_forecast(filename, wtd):
    ...
    scaled = scaler.fit_transform(daily[['flood runoff']])
    scale = pd.DataFrame(scaled, columns=['flood runoff'], index=daily.index)
    ...
    plt.title('Flood Runoff Forecast')
...
# Example usage:
# flood_runoff_forecast('your_filename', wtd=0)  # Set wtd to 0 for testing, 1 for future prediction
