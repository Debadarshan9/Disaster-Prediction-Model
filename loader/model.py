...
def flood_classifier(filename, fd, validating=0):
    ...
    # Filling null entries with the mean of their respective columns
    for i in range(1, len(data.columns)):
        data[data.columns[i]] = data[data.columns[i]].fillna(data[data.columns[i]].mean())

    # Converting 'Flood' column to binary labels
    y = data['Flood']
    y = y.apply(lambda x: 1 if x >= 0.1 else 0)
    ...
    locate = data[data['Year'] == 2015].index[-1] + 1
    x_train = data.iloc[:locate]
    y_train = y.iloc[:locate]
    x_test = data.iloc[locate:]
    y_test = y.iloc[locate:]
    ...
    return y_pred, mae, classification_rep


# Example usage:
# fd = [4990.0, 0.0, 1.40, 15.38]  # Sample future data
# flood_classifier('your_filename', fd, validating=0)
