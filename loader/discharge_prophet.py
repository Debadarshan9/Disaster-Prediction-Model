...
def discharge_forecast(filename, wtd):
    # Import raw data
    def import_data():
        data_df = pd.read_excel('data/' + filename + '.xlsx', header=0)  # creates a Pandas data frame for input value
        return data_df
    ...
    scaled = scaler.fit_transform(daily[['Discharge']])
    scale = pd.DataFrame(scaled, columns=['Discharge'], index=daily.index)
    ...
    # Getting the values in original range
...
# Example usage:
# discharge_forecast('your_filename', wtd=0)  # Set wtd to 0 for testing, 1 for future prediction
